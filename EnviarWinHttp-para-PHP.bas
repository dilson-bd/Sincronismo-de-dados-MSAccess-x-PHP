             ...
             ' MsgBox k(0) & " - " & k(1) & " - " & k(2)
                   If IsNumeric(k(1)) = True Then
                        'MsgBox "NOVO - Atualizarei salvando o codweb"
                         CurrentDb.Execute "UPDATE Paciente SET CodWeb_a=" & k(1) & ", SubirDepois_a=0 WHERE CódigoDoPaciente=" & argCodPaciente
                   ElseIf k(1) = "atualizado" Then
                        'MsgBox "Já EXISTE - Atualizarei salvando Zero em SubirDepois"
                         CurrentDb.Execute "UPDATE Paciente SET CodWeb_a=" & k(2) & ", SubirDepois_a=0 WHERE CódigoDoPaciente=" & argCodPaciente
                   End If
sair:
    Exit Function
trataErro:
...
Public Function WebEnviarLancamento(argEstacao As String, argCodWeb As Long, argCTLrelacaoA_e As String, argCTLrelacaoB_p As String, _
                                    argCódigoDoLançamento As Long, argDataDaTransação As String, argCódigoDoPaciente As Long, _
                                    ...
                                    argDataDaSolic As String, argPrioridade As Integer, argMês As String, argAno As String, argCodigoDoProced As String, argDescrCodProced As String, _
                                    ...
                                    argDataMarcarRet As String, argMesRetorno As String, argCid10 As String, argDescricaoCID10 As String, argLocalArq As String, argAnotações As String, _
    ...
    '-------------------------------- Fim winhttp 1 de 2
             ...
             '----------------------------------------------- winhttp 2 de 2
              WinHttpReq.Open "GET", qURL _
                                   ...
                                   & "&codlanc=" & argCódigoDoLançamento & "" _
                                   & "&dataproc=" & argDataDaTransação & "" _
                                   & "&codpac=" & argCódigoDoPaciente & "" _
                                   & "&pac=" & argPaciente & "" _
                                   ...
                                   & "&prioridade=" & argPrioridade & "&mes=" & argMês & "&ano=" & argAno & "" _
                                   ...
                                   & "&localarq=" & argLocalArq & "&anotacoes=" & argAnotações & "&protsms=" & argProtSMSEnvioAg & "&fechamento=" & argDataFechamento & "&alias=" & argAlias, False
              WinHttpReq.SetRequestHeader "Content-Type", "text/html; charset=UTF-8"
              WinHttpReq.Send
              WinHttpReq.WaitForResponse
              strResult = WinHttpReq.responseText
             '----------------------------------------------- Fim winhttp 2 de 2
              ...
              'MsgBox k(0) & " - " & k(1) & " - " & k(2)
                   If IsNumeric(k(1)) = True Then
                        'MsgBox "NOVO - Atualizarei salvando o codweb"
                         CurrentDb.Execute "UPDATE Lançamentos SET CodWeb_b=" & k(1) & ", SubirDepois_b=0 WHERE CódigoDoLançamento=" & argCódigoDoLançamento
                   ElseIf k(1) = "atualizado" Then
                        'MsgBox "Já EXISTE - Atualizarei salvando Zero em SubirDepois"
                         CurrentDb.Execute "UPDATE Lançamentos SET CodWeb_b=" & k(2) & ", SubirDepois_b=0 WHERE CódigoDoLançamento=" & argCódigoDoLançamento
                   End If
sair:
    Exit Function
trataErro:
    ...
    '-------------------------------- Fim winhttp 1 de 2
             ...
             '----------------------------------------------- Fim winhttp 2 de 2
              ...
              'MsgBox k(0) & " - " & k(1) & " - " & k(2)
                   If IsNumeric(k(1)) = True Then
                        'MsgBox "NOVO - Atualizarei salvando o codweb"
                         CurrentDb.Execute "UPDATE tblUsuários SET CodWeb_a=" & k(1) & ", SubirDepois_a=0 WHERE IdUsuario=" & argIdUsuario
                   ElseIf k(1) = "atualizado" Then
                        'MsgBox "Já EXISTE - Atualizarei salvando Zero em SubirDepois"
                         CurrentDb.Execute "UPDATE tblUsuários SET CodWeb_a=" & k(2) & ", SubirDepois_a=0 WHERE IdUsuario=" & argIdUsuario
                   End If
sair:
    Exit Function
trataErro:
...
Call defineURL
   ...
   '1<<
         ...
         rs.MoveFirst
         Do While Not rs.EOF
            qChavePrimaria = rs("CódigoDoPaciente")
             DoCmd.Hourglass (True)
             'Definir 10 segundos time-outs. Se os tempos limite estiverem definidos, eles devem
              'ser definida antes de abrir.
              WinHttpReq.SetTimeouts lngTimeout, lngTimeout, lngTimeout, lngTimeout
             '----------------------------------------------- winhttp 2 de 2
              WinHttpReq.Open "GET", qURL _
                                   ...
                                   & "&codpac=" & Nz(rs!CódigoDoPaciente) & "" _
                                   & "&rg=" & Nz(rs!RG) & "" _
                                   ...
                                   & "&resp=" & Nz(rs!NomeDoResponsável) & "" _
                                   & "&naturalidade=" & Nz(rs!Naturalidade) & "" _
                                   & "&estadocivil=" & Nz(rs!EstadoCivil) & "" _
                                   & "&endereco=" & Nz(rs!Endereço) & "" _
                                   & "&nr=" & Nz(rs!N°) & "" _
                                   & "&cep=" & Nz(rs!Cep) & "" _
                                   & "&telprinc=" & Nz(rs!TelPrinc) & "" _
                                   & "&telalt=" & Nz(rs!TelAlt) & "&bairro=" & Nz(rs!Bairro) & "&municipio=" & Nz(rs!MunicípioDeReside) & "" _
             ...
             '----------------------------------------------- Fim winhttp 2 de 2
              ...
              'MsgBox k(0) & " - " & k(1) & " - " & k(2)
                   ...
                   Forms!FCProcessos!rotProgresso.Caption = "Inserindo Paciente CodWeb " & k(1)
                          'MsgBox "NOVO - Atualizarei salvando o codweb"
                          CurrentDb.Execute "UPDATE Paciente SET CodWeb_a=" & k(1) & ", SubirDepois_a=0 WHERE CódigoDoPaciente=" & qChavePrimaria
                   ElseIf k(1) = "atualizado" Then
                   Forms!FCProcessos!rotProgresso.Caption = "Atualizando Paciente CodWeb " & k(2)
                          'MsgBox "JÁ EXISTE - Atualizarei salvando Zero em SubirDepois"
                          CurrentDb.Execute "UPDATE Paciente SET CodWeb_a=" & k(2) & ", SubirDepois_a=0 WHERE CódigoDoPaciente=" & qChavePrimaria
                   End If
              rs.MoveNext
              Loop
              DoCmd.Hourglass (False)
...
Call defineURL
    ...
    '-------------------------------- winhttp 1 de 2
      ...
      Set WinHttpReq = CreateObject("WinHttp.WinHttpRequest.5.1")
    '-------------------------------- Fim winhttp 1 de 2
     'stSQL = "SELECT TOP 1000, * FROM Lançamentos "
     stSQL = "SELECT * FROM Lançamentos " _
            & "WHERE " _
            & "(((SubirDepois_b)=-1));"
      Set rs = CurrentDb.OpenRecordset(stSQL)
   If rs.RecordCount > 0 Then
   '1<<
         ...
         rs.MoveFirst
         Do While Not rs.EOF
         qChavePrimaria = rs("CódigoDoLançamento")
         'MsgBox qChavePrimaria
             ...
             '----------------------------------------------- winhttp 2 de 2
                            WinHttpReq.Open "GET", qURL _
                                   ...
                                   & "&codlanc=" & Nz(rs!CódigoDoLançamento) & "" _
                                   & "&dataproc=" & Nz(Format(rs!DataDaTransação, "yyyy/mm/dd")) & "" _
                                   & "&codpac=" & Nz(rs!CódigoDoPaciente) & "" _
                                   ...
                                   & "&prioridade=" & Nz(rs!Prioridade) & "&mes=" & Nz(rs!Mês) & "&ano=" & Nz(rs!Ano) & "" _
                                   ...
                                   & "&localarq=" & Nz(rs!Redund_Local) & "&anotacoes=" & Nz(rs!Anotações) & "&protsms=" & Nz(rs!ProtSMSEnvioAg) & "&fechamento=" & Nz(Format(rs!DataFechamento, "yyyy/mm/dd")) & "&alias=" & Nz(qualAlias), False
              WinHttpReq.SetRequestHeader "Content-Type", "text/html; charset=UTF-8"
              WinHttpReq.Send
              WinHttpReq.WaitForResponse
              strResult = WinHttpReq.responseText
             '----------------------------------------------- Fim winhttp 2 de 2
              ...
              'MsgBox k(0) & " - " & k(1) & " - " & k(2)
                   ...
                   If IsNumeric(k(1)) = True Then
                   Forms!FCProcessos!rotProgresso.Caption = "Inserindo Lançamento CodWeb " & k(1)
                          'MsgBox "Atualizarei salvando o codweb"
                          CurrentDb.Execute "UPDATE Lançamentos SET CodWeb_b=" & k(1) & ", SubirDepois_b=0 WHERE CódigoDoLançamento=" & qChavePrimaria
                   ElseIf k(1) = "atualizado" Then
                   Forms!FCProcessos!rotProgresso.Caption = "Atualizando Lançamento CodWeb " & k(2)
                          'MsgBox "Atualizarei salvando Zero em SubirDepois"
                          CurrentDb.Execute "UPDATE Lançamentos SET CodWeb_b=" & k(2) & ", SubirDepois_b=0 WHERE CódigoDoLançamento=" & qChavePrimaria
                   End If
              rs.MoveNext
              Loop
              DoCmd.Hourglass (False)
...
Call defineURL
    ...
    '-------------------------------- winhttp 1 de 2
      ...
      Set WinHttpReq = CreateObject("WinHttp.WinHttpRequest.5.1")
    '-------------------------------- Fim winhttp 1 de 2
     stSQL = "SELECT * FROM tblUsuários " _
            & "WHERE " _
            & "(((SubirDepois_a)=-1));"
      Set rs = CurrentDb.OpenRecordset(stSQL)
   If rs.RecordCount > 0 Then
            ...
            qChavePrimaria = rs("IdUsuario")
             ...
             '----------------------------------------------- Fim winhttp 2 de 2
              ...
              'MsgBox k(0) & " - " & k(1) & " - " & k(2)
                   If IsNumeric(k(1)) = True Then
                          'MsgBox "NOVO - Atualizarei salvando o codweb"
                          CurrentDb.Execute "UPDATE tblUsuários SET CodWeb_a=" & k(1) & ", SubirDepois_a=0 WHERE IdUsuario=" & qChavePrimaria
                   ElseIf k(1) = "atualizado" Then
                          'MsgBox "JÁ EXISTE - Atualizarei salvando Zero em SubirDepois"
                          CurrentDb.Execute "UPDATE tblUsuários SET CodWeb_a=" & k(2) & ", SubirDepois_a=0 WHERE IdUsuario=" & qChavePrimaria
                   End If
              rs.MoveNext
              Loop
              DoCmd.Hourglass (False)
...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
'OBJETIVO: Marcar SubirDepois em registros que estão com o CodWeb_(letra) vazio e SubirDepois = 0 como se o registro
...
On Error GoTo trataErro
    ...
    '-------------------------------- Fim winhttp 1 de 2
              DoCmd.Hourglass (True)
             '----------------------------------------------- winhttp 2 de 2
             'Mensagem tem que ter no máximo 160 caracteres
              WinHttpReq.Open "GET", qURL _
...
'///////////////////////////////////////////////////////////////////////////////////////
'Código para ir buscar os dados de envio sms na tabela configsms
